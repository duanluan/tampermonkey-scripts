...
export default class Options {
  ...
  static loadInV2ex() {
    if (location.href.indexOf('v2ex.com/settings') == -1) {
      // 可以直接用 location.pathname.startsWith('/settings')
      return;
    }
    const selector = {
        menuTab: 'a.tab',
        ...
        <form>
          <div class="inner">
            ...
            <div class="social-settings-form">
              <div>
                <div>V2EX Token</div>
                <input type="password" class="sl" name="v2exToken" value="${
                  JSON.parse(Store.get(this.Keys.v2exToken)).value
                }">
              </div>
            </div>
          </div>
