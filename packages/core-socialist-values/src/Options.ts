...
export default class Options {
  ...
  static loadInGreasyfork() {
    CommonOptions.loadInGreasyfork(this.SCRIPT_ID, ($scriptContent: any) => {
      // 添加脚本设置的内容
      let scriptContent = '';
      for (const option of this.options) {
        const storeOption = JSON.parse(Store.get(option.name)),
        ...
        switch (optionName) {
          case this.Keys.removeBar:
            scriptContent += `<label><input type="checkbox" id="script-options-${optionName}" ${
              optionVal ? 'checked' : ''
            }> 是否隐藏条幅（勾选后将隐藏条幅而不是替换其内容）</label>`;
            break;
          case this.Keys.jinrishici:
            scriptContent += `<label><input type="checkbox" id="script-options-${optionName}-enabled" ${
              optionVal ? 'checked' : ''
            }> 启用<a href="https://www.jinrishici.com" target = "_blank">今日诗词</a>（仍需取消勾选“是否隐藏条幅”才能生效）</label>`;
            break;
        }
      }
