...
export default class Bar {
   ...
   */
  private static txt =
    '富强民主文明和谐自由平等公正法治爱国敬业诚信友善ProsperityDemocracyCivilityHarmonyFreedomEqualityJusticeRuleoflawPatriotismDedicationIntegrityFriendship';
  /**
   * 是否启用今日诗词
   * @private
   */
  ...
  static replace(options: {
    barSelector: string;
    isObserveBar: boolean;
    followUpObserveSelector?: string;
    hideBarSelector: string;
    isObserveHideBar: boolean;
    replaceBarCallback?: Function;
    jinrishiciCallback?: Function;
    hideBarCallback?: Function;
  }) {
    if (
      !$(options.barSelector)
        .text()
        .match(/ukraine|乌克兰|black|黑人/i)
    ) {
      return;
    }
   ...
   * @private
   */
  private static replaceObserver(options: {
    barSelector: string;
    isObserveBar: boolean;
    hideBarSelector: string;
    isObserveHideBar: boolean;
