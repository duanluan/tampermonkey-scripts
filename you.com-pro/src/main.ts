...
(() => {
  ...
  function init() {
    ...
    $(document.body).append(`
      <div id="${selectorId.proOperatePanel}" style="display: none; background: #eeeef1; padding: 10px; border: 1px solid #e5e5e5; position: absolute; z-index: 99999; right: 0; top:${proOperatePanelTop + 10}px; width: 270px; height: calc(100vh-${proOperatePanelTop}px)">
        <button type="button" class="layui-btn layui-btn-sm" id="${selectorId.inputScriptBtn}">填写脚本</button>
      </div>
    `)
    ...
    // 填写脚本按钮点击事件
    $(selector.inputScriptBtn)[0].addEventListener('click', () => {
      layer.prompt({
        title: '自己账号需先禁用脚本并注销！',
        maxlength: 10000
      }, (value, index) => {
