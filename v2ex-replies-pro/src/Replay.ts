...
export default class Replay {
  ...
  static async run() {
    ...
    const showReplies = () => {
      db.transaction(tx => {
        // @formatter:off
        tx.executeSql(`select * from rendered_replies order by created`, [], (tx, results) => {
        // @formatter:on
    ...
    const showTaSay = (replyDivId: string, username: string, created: number) => {
      ...
      db.transaction(tx => {
        // @formatter:off
        tx.executeSql(`select * from rendered_replies where username = ? order by created`, [username],(tx, results) => {
        // @formatter:on
